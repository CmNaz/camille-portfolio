        ...
        if self.close15!=0:
            ...
            if self.shortClosed!=None:
                ...
                if self.close15/self.shortClosed<=0.985 and self.strdownTrend and not self.short_position and upmidDistance<-1.5 and middownDistance<-1.5:
                    ...
                    hoh=c.open_short(self.SHORT_QUANTITY_START, self.CRYPT, self.TRADE_SYMBOL)
                    self.newShort=False
                    if hoh:
            ...
            if (not self.upTrend and self.long_position) or (self.long_position and self.profit<0.972):
                ...
                hoh=c.stop_loss(self.upTrend, self.SHORT_QUANTITY_START, self.SHORT_QUANTITY_END, self.LONG_QUANTITY, self.TRADE_SYMBOL, self.TRADE_SYMBOL)
                if hoh:
                    self.newLong=False
                    self.long_position=False
            ...
            if (self.upTrend and self.short_position) or (self.short_position and self.profitShort<0.972):
                ...
                hoh=c.stop_loss(self.upTrend, self.SHORT_QUANTITY_START, self.SHORT_QUANTITY_END, self.LONG_QUANTITY, self.CRYPT, self.TRADE_SYMBOL)
                if hoh:
                    self.newShort=False
                    self.short_position=False
                    self.shortCont=False
                ...
                if self.upTrend and self.instanceLong!=0:
                    ...
                    if self.short_position:
                        ...
                        if (self.profitShort > 1.022) or (last_rsi_6<15 and not self.shortCont):
                            ...
                            hoh=c.close_short(self.SHORT_QUANTITY_START, self.SHORT_QUANTITY_END, self.CRYPT, self.TRADE_SYMBOL)
