...
class viye:
    ...
    def do(self):
        ...
        print("All 1-MIN RSIs Calculated Thus Far:\r")
        print(rsi6[-5:])
        print(rsi12[-5:])
        print(rsi24[-5:])
        ...
        if self.close15!=0:
            ...
            if self.shortClosed!=None:
                ...
                if self.close15/self.shortClosed<=0.99 and self.strdownTrend and not self.short_position and upmidDistance<-1.5 and middownDistance<-1.5:
                    #enter short
                    self.SHORT_QUANTITY_START=c.round_up(self.STABLE_SHORT/self.close15)
                    self.shortOpened=self.close15
                    hoh=c.open_short(self.SHORT_QUANTITY_START, self.TRADE_SYMBOL, self.CRYPT, self.close15)
                    self.newShort=False
                    if hoh:
            ...
            if self.closed!=None:
                ...
                if self.closed/self.close15<=0.99 and self.strupTrend and not self.long_position and upmidDistance>2 and middownDistance>2:
                    ...
                    order_successful= c.order_buy(self.LONG_QUANTITY, self.TRADE_SYMBOL, self.close15, self.CRYPT)
            ...
            if (not self.upTrend and self.long_position) or (self.long_position and self.profit<0.988) or (c.change and self.long_position):
            ...
            if (self.upTrend and self.short_position) or (self.short_position and self.profitShort<0.98) or (c.change and self.short_position):
            ...
            if (upmidDistance<-1.5 and middownDistance<-1.5 and last_rsi_24<45) or self.newLong:#this newLong is actually useless because of the conditions for profitrev and last rsi
                if self.upTrend and self.instanceLong!=0:
                    if self.long_position:
                        ...
                        if (self.profitRev>1.015) or (last_rsi_24<43) or (self.newLong):
                            print("Attempting to enter long position!\r")
                            c.long_buy_hist(last_rsi_6, last_rsi_12, last_rsi_24)
                    ...
                    if self.short_position:
                        #if last_rsi_6>60
                        #YOU CHANGED DOWNTREND PROFIT FROM 2.2% TO 1.7% 
                        if (self.profitShort > 1.0125) or (last_rsi_6<23 and not self.shortCont): #and self.profitShort>1.01
                            self.SHORT_QUANTITY_END=c.round_up(self.STABLE_SHORT/self.close15)
        ...
        self.statement1=("==================================================================================="
                +"\n\nAll 1-MIN RSIs Calculated Thus Far: \n 1-MIN RSI 6: {}".format(rsi6[-5:])+"\n 1-MIN RSI 12: {}".format(rsi12[-5:])+"\n 1-MIN RSI 24: {}".format(rsi24[-5:])
        +"\n\n Current 1-MIN RSI 6 is: {}".format(last_rsi_6)+"\nCurrent 1-MIN RSI 12 is: {}".format(last_rsi_12)+"\nCurrent 1-MIN RSI 24 is: {}".format(last_rsi_24)+"\nValue of UpTrend: {}".format(self.upTrend)
        ...
        print("Current 1-MIN RSI 6 is: {}".format(last_rsi_6))
        print("Current 1-MIN RSI 12 is: {}".format(last_rsi_12))
        print("Current 1-MIN RSI 24 is: {}".format(last_rsi_24))
